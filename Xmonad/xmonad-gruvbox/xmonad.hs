...
-- Custom function to toggle window floating state
...
myLayouts = avoidStruts $
  ...
  where
    ...
    baseTiled = ResizableTall 1 (3/100) (1/2) []

    
    tiledRenamed = renamed [Replace "Tall"] baseTiled
    mirrorTiledRenamed = renamed [Replace "Wide"] (Mirror baseTiled)

    
    tiledLayouts = spacingRaw False (Border 24 4 4 4) False (Border 4 4 4 4) True $
                      tiledRenamed ||| mirrorTiledRenamed

    full = renamed [Replace "Full"] Full

    
    grid = renamed [Replace "Grid"] $ spacingRaw False (Border 24 4 4 4) False (Border 4 4 4 4) True Grid

  
    tabbedLayout = renamed [Replace "Tabbed"] $ noBorders $ tabbed shrinkText myTabTheme
...
main = do
  ...
  xmonad $ docks . ewmhFullscreen . ewmh $ def
    ...
    , ((mod4Mask .|. shiftMask, xK_c), kill)
      ...
      -- Toggle Floating/Tiling state 
    , ((mod4Mask, xK_t), withFocused toggleFloat)

      -- Audio controls (volume and mute)
    , ((0, xF86XK_AudioLowerVolume), spawn "wpctl set-volume @DEFAULT_AUDIO_SINK@ -5% && notify-send 'Volume' 'Decreased by 5%'")
    , ((0, xF86XK_AudioRaiseVolume), spawn "wpctl set-volume @DEFAULT_AUDIO_SINK@  5%+ && notify-send 'Volume' 'Increased by 5%'")
    , ((0, xF86XK_AudioMute), spawn "wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle && notify-send 'Volume' 'Muted/Unmuted'")
